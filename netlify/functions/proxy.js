...
// IMPORTANTE: Apps Script responde a un POST en /exec con un redirect 301/302/303
// hacia una URL firmada de un solo uso en script.googleusercontent.com que
// entrega el contenido real de la respuesta. Ese salto SIEMPRE debe seguirse
// con GET (nunca reenviando el body original) — así es como los navegadores
// tratan un 301/302/303 en la práctica y así es como Apps Script espera que
// se siga. Antes reenviábamos el POST completo (con el archivo) al redirect,
// lo cual regresaba una página HTML en vez del JSON real.
          ...
          const req = https.request(options, (res) => {
                  if ([301,302,303,307,308].includes(res.statusCode)) {
                            ...
                            if ([307,308].includes(res.statusCode)) {
                                        // 307/308 exigen preservar método y body por spec
                              postWithRedirects(location, body, maxRedirects - 1, timeoutMs).then(resolve).catch(reject);
                            } else {
                                        // 301/302/303: seguir con GET, sin body (comportamiento estándar de navegador
                              // y lo que Apps Script realmente espera para entregar el contenido)
                              getWithRedirects(location, maxRedirects - 1).then(resolve).catch(reject);
                            }
                            return;
                  }
                  let data = '';
