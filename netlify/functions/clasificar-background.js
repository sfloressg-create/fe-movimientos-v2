...
// Procesa el PDF con Claude y guarda el resultado en Netlify Blobs.
...
exports.handler = async function (event) {
  ...
  try {
    ...
    // El navegador genera el jobId y lo manda; si no lo manda, lo generamos aquí
    jobId = body.jobId || ("job_" + Date.now() + "_" + Math.random().toString(36).substring(2, 10));

    if (!apiKey || !fileBase64) {
      await store.setJSON(jobId, { ok: false, status: "error", error: "Falta apiKey o archivo" });
      return;
    }

    // Marcamos como "procesando" de inmediato para que el polling no encuentre un vacío confuso
    await store.setJSON(jobId, { ok: true, status: "procesando" });
...
Si una fecha no tiene año explícito, usa el año que aparece en el encabezado del estado de cuenta.`;
    ...
    if (!textBlock) {
      await store.setJSON(jobId, { ok: false, status: "error", error: "Claude no devolvió texto" });
      return;
    }
    ...
    await store.setJSON(jobId, { ok: true, status: "listo", movimientos: movimientos });

  } catch (err) {
    if (jobId) {
      const store2 = getStore("clasificaciones");
      await store2.setJSON(jobId, { ok: false, status: "error", error: err.message });
    }
  }
};
